...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const [skill, chatbotTypes] = await Promise.all([
    db.skill.findUnique({
  ...
  return json({
    skill,
    chatbotTypes,
    selectedChatbotTypeIds: skill.chatbotTypes.map(ct => ct.chatbotTypeId)
  });
}
...
export default function AssignChatbotTypes() {
  const { skill, chatbotTypes, selectedChatbotTypeIds } = useLoaderData<typeof loader>();
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await fetch(`/admin/skill/${skill.id}/assign-chatbot-types`, {
        method: 'POST',
        headers: {
      ...
      toast.success(t("admin.skill.chatbotTypesAssigned"));
      navigate('/admin/skill');
    } catch (error) {
      toast.error(t("admin.skill.chatbotTypesAssignError"));
      setIsSaving(false);
    }
  };
    ...
    <div className="container mx-auto py-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold">
          {t("admin.skill.assignChatbotTypes", { skill: skill.name })}
        </h1>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>{t("admin.skill.selectChatbotTypes")}</CardTitle>
        </CardHeader>
        <CardContent>
